...
def f_default_arg_mutable(x: int, data: List[int] = []):
    if not data:
        data = [x]
    elif len(data) < 3:
        data.append(x)
...
def f_indexing_and_slicing(data: List[int]) -> Tuple[int, int, List[int]]:
    my_tuple = data[0], data[-1], data[0:-1]
    return my_tuple
...
def f_str_formatting(array: float):
    i = 0
    result = ''
    while i < len(array):
        if array[i] == array[-1]:
            result = result + "{:.2f}".format(array[i])
        else:
            result = result + "{:.2f} - ".format(array[i])
        i = i + 1
    return result
...
def f_logic(x: int, b1: bool, b2: bool):
    if (1 < x < 4) or (b1 is True and b2 is False):
        return True
...
def f_comprehensions(dct: Dict[int, int]) -> Tuple[int, Set[int]]:
    ad1 = sum([a for a in range(2, 11) if a not in [5, 7]])
    ad2 = set([b for b, c in dct.items() if 1 <= c <= 5])
    result = (ad1, ad2)
    return result
